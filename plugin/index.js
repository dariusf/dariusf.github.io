	...
	for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
		...
		if (entry.isFile() && entry.name.endsWith(".md")) {
			...
			for (const f of fs.readdirSync(full, { recursive: true })) {
				...
				page.assets.push({
					src,
					dest: path.posix.join(prefix.slice(1), entry.name, f),
				});
			}
			pages.push(page);
		}
...
function nunjucksEnv(root) {
	...
	env.addFilter("htmlDateString", (d) =>
...
export function createSite(root) {
			...
			.map((p) => {
				...
				if (p.engine === "md") {
					...
					Object.defineProperty(entry, "content", {
						get: () =>
							(html ??= md.render(p.body, {
								mathDefs: p.fm.mathDefs,
								page: p.url,
							})),
					});
				}
	...
	function renderPage(page) {
		...
		let content;
		if (page.engine === "md") {
			content = md.render(page.body, {
				mathDefs: page.fm.mathDefs,
				page: page.url,
			});
		} else {
...
export function renderSite(root) {
	...
	for (const page of site.pages) {
		...
		for (const asset of page.assets)
			copyFile(asset.src, path.join(out, asset.dest));
	}
